...
def create_table(table_df, title, id, null_list):
    data = [html.H4(title, style={'textAlign':'center'})]
...
def show_tables(checked):  # shows divs with tables in them if checked
    ...
    if checked:
        for item in checked:
            dic[item] = {'display': 'block', 'margin-bottom':'25px'}
...
def make_tables(dic, name):
    ...
    full_table = create_table(dff[FULL_COLS], 'Full Table', 'custom-table', strings)
