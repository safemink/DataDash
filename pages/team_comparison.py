...
def mean_sum_dfs(df):
    ...
    return {'avs': [av_team, av_opp], 'tots': [tot_team, tot_opp]}
...
#     return df1, df2
def stat_func_filter(dic, func_name, cols):
    if func_name == 'Average':
        avs = dic['avs']
        df1 = avs[0][cols]
        df2 = avs[1][cols]
...
def team_comp_table(table_df, data_style, id):
...
def show_table(table_type, func_name, legend):  # takes in a string of gen, shots, goals, ex and either mean or sum
    ...
    df1, df2 = stat_func_filter(df_dic, func_name, filter_cols)
    ...
    table = team_comp_table(table_df, style, f'{table_type}-{func_name}') #### breaks real hard here
